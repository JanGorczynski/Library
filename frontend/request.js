 ...
 async function  getBooksByUsername(username){
    const base = "http://localhost:8080/Rents";
...
async function  removeBooksById(id){
    const base = "http://localhost:8080/Rents/delete";
    const query = `?id=${id}`;
...
async function  prolongById(id){
    const base = "http://localhost:8080/Rents/prolong";
    const query = `?id=${id}`;
    console.log(base+query);
    fetch(base+query, {
        method : 'PATCH'
...
async function addUser(username,password){
    const base = "http://localhost:8080/user/add";
...
async function getAvailableBooks(){
    const query = "http://localhost:8080/books";
